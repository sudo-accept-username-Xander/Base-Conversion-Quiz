...
def Make_Easy(): # This makes the easy difficulty
    ...
    global answer, points, easy_result_label, easy_window, load_question, question_no
    ...
    easy_hint_label = Label(easy_window, text="Hint: Base 10 is normal numbers.", font=("Arial", 10), bg=easy_window_bg)
    easy_hint_label.grid(row=9, column=3, columnspan=2)

    question_label = Label(easy_window, font=("Arial", 11), text="", bg=easy_window_bg)
    question_label.grid(row=4, column=1, columnspan=3)

    easy_user_entry = Entry(easy_window, font=("Arial", 12), width=12)
    easy_user_entry.grid(row=5, column=2)
    easy_user_entry.bind("<Return>", lambda event: check_answer("easy"))

    easy_submit_button = Button(easy_window, text="Submit", command=lambda: check_answer(easy_user_entry, "easy"))
    ...
    def load_question():
        ...
        if this_to == 1: # Binary to
            ...
            if this == 1: # Decimal
                ...
                answer = decimal_int
                question_label.config(text="What is "+str(binary_int)+" (binary) in base 10")
            ...
            if this == 1: # Binary
                ...
                binary_int = int(binary)
                answer = binary_int
                question_label.config(text="What is "+str(decimal_int)+" in binary")
            elif this == 2: # Octal (cant be called on easy)
                error("outside_val")
            elif this == 3: # Hex (cant be called on easy)
                error("outside_val")
...
def check_answer(entry, difficulty, event=None):
    global answer, points, easy_result_label, easy_window, medium_window, expert_window, load_question
