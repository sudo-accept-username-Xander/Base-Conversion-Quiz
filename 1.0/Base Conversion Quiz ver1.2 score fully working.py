...
def error(error_val):
    ...
    if error_val == "outside_val":
        messagebox.showerror("Error", "outside_val")
    else:
        messagebox.showerror("Error", "UNDEFINED ERROR")
...
def write_score(difficulty, score, time_taken, user):
            ...
            for record in records:
                try:
                    ...
                    if diffi.strip().lower() == difficulty.lower():
                        filtered.append((int(score), time_taken, user.strip(), score, time_taken))
                except ValueError:
                    continue

            # Sort by score then fastest time
            sorted_scores = sorted(filtered, key=lambda x: (-x[0], x[1]))[:10]

            if sorted_scores:
                for row_no, (_, _, user, score, time_taken) in enumerate(sorted_scores):
...
def really_quit(): # Askes the user if they really want to quit (this is not called on file->exit)
    ...
    if quit_window_open == False:
        ...
        no_button = Button(quit_window, text="No", command=quit_from_here, width=10, bg=easy_window_bg).grid(row=1, column=3)
